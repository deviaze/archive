...
impl ArchiveBuilder {
    ...
    /// # }
    /// ```
    pub fn build(&self, entries: Vec<ArchiveEntry>, format: ArchiveFormat) -> Result<Vec<u8>> {
        match format {
    ...
    pub fn build_single_file(
        ...
        match format {
            ...
            | ArchiveFormat::Xz
            | ArchiveFormat::Lz4
            | ArchiveFormat::Zst => self.build(vec![ArchiveEntry::file(path, data)], format),
    ...
    fn build_zip(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_tar(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let mut builder = tar::Builder::new(Vec::new());
        Self::write_tar_entries(&mut builder, entries)?;
        Ok(builder.into_inner()?)
    }

    fn build_tar_gz(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_tar_bz2(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_tar_zst(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_tar_lz4(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_tar_xz(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    fn build_ar(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        ...
        for entry in entries {
            match entry {
                ArchiveEntry::File { path, data } => {
                    validate_path(&path)?;
                    let header = ar::Header::new(path.into_bytes(), data.len() as u64);
    ...
    fn build_7z(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
    ...
    /// [`ArchiveEntry::File`].
    fn single_file_entry(entries: Vec<ArchiveEntry>) -> Result<(String, Vec<u8>)> {
        let [entry] = <[ArchiveEntry; 1]>::try_from(entries).map_err(|entries| {
    ...
    fn build_single_gz(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let (path, data) = Self::single_file_entry(entries)?;
    ...
    fn build_single_bz2(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let (_path, data) = Self::single_file_entry(entries)?;
        let mut encoder = bzip2::write::BzEncoder::new(Vec::new(), bzip2::Compression::default());
        encoder.write_all(&data)?;
        Ok(encoder.finish()?)
    }

    fn build_single_xz(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let (_path, data) = Self::single_file_entry(entries)?;
        let mut output = Vec::new();
    ...
    fn build_single_lz4(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let (_path, data) = Self::single_file_entry(entries)?;
    ...
    fn build_single_zst(&self, entries: Vec<ArchiveEntry>) -> Result<Vec<u8>> {
        let (_path, data) = Self::single_file_entry(entries)?;
